...
class CacheFeedUseCaseTests: XCTestCase {
    ...
    func test_save_requestsNewCacheInsertionWithTimestampOnSuccessfulDeletion() {
        let timestamp = Date()
        let items = [uniqueItem(), uniqueItem()]
        let localFeedItems = items.map { LocalFeedItem(id: $0.id, location: $0.location, description: $0.description, imageURL: $0.imageURL) }
    ...
    private func expect(_ sut: LocalFeedLoader, toCompleteWithError expectedError: Error?, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
